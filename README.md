...
[![Тесты](https://img.shields.io/badge/%D1%82%D0%B5%D1%81%D1%82%D1%8B-2334%20%2B%2011-729B1B?logo=jest&logoColor=white)](#проверка)
...
```bash
npm test            # 2334 юнит-теста в 39 наборах
npm run test:e2e    # 11 сквозных сценариев Cypress поверх dev-сервера
npm run lint
npx tsc --noEmit
```
...
и в тени; средние — на случаях из регламента.
...
**CI:** на каждом PR в `main` две обязательные проверки — `audit`
(`audit-ci --high`, без исключений) и `build` (линт, типы, юнит-тесты, сборка и
проверка всего Docker-стека).
