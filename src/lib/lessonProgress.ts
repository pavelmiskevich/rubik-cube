 ...
 *
 * Правило хранения намеренно простое: вошёл — база, не вошёл — localStorage.
 * Слияния при входе нет (отдельная задача эпика #51). Этот модуль не знает,
 * куда пишется прогресс: он решает, что писать, и проверяет то, что прочитано.
